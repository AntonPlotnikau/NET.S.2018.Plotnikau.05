...
namespace Extension.Tests
{
    ...
    class StringExtensionsTests
    {
        ...
        public static long ToDecimalFormTests(string source, int systemIndex)
            => source.ToDecimalForm(new StringExtensions.Notation(systemIndex));

        [Test]
        [TestCase("1AeF101", 2)]
        [TestCase("SA123", 2)]
        public static void ToDecimalFormArgumentException(string source, int systemIndex)
            => Assert.Throws<ArgumentException>(() => source.ToDecimalForm(new StringExtensions.Notation()));
        ...
        public static void ToDecimalFormOverflowException(string source, int systemIndex)
            => Assert.Throws<OverflowException>(() => source.ToDecimalForm(new StringExtensions.Notation()));
    }
}
